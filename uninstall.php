 ...
 *
 *     if ( '1' !== get_option( 'wpkernel_delete_data_on_uninstall' ) ) {
 *         return;
 *     }
 *
 * The example below drops the schema-version and migration-tracking
 * options unconditionally because they are WPKernel's own bookkeeping,
 * not plugin data — replace/extend this once you have real tables.
 */
delete_option( 'wpkernel_db_version' );
delete_option( 'wpkernel_applied_migrations' );
